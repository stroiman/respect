...
function run(grp, parents) {
  ...
  var partial_arg = Async.all(List.map((function (grp) {
              return run(grp, groupStack);
            }), grp[/* children */1]));
  var childGroupResult = function (param) {
    return Async.map((function (param) {
                  return List.fold_left(merge, empty, param);
                }), partial_arg, param);
  };
  var exampleResults = iter(empty, grp[/* examples */3]);
  return (function (param) {
      return Async.map((function (param) {
                    return merge(param[0], param[1]);
                  }), (function (param) {
                    return Async.both(exampleResults, childGroupResult, param);
                  }), param);
    });
}
