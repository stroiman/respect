...
let rec applyOperation = (operation, context, metadata, focus, skipped) =>
  switch operation {
  | AddOperator(Focus, op) => applyOperation(op, context, metadata, true, skipped)
  | AddOperator(Skip, op) => applyOperation(op, context, metadata, focus, true)
  | WrapMetadata((key, value), op) => applyOperation(op, context, metadata |> Respect_ctx.ContextMap.add(key, value), focus, skipped)
  ...
  | AddChildGroupOperation(name, ops) =>
    ...
    let newChild = List.fold_left((grp, op) => applyOperation(op, grp, Respect_ctx.ContextMap.empty, focus, skipped), initial, ops);
...
let applyOperation = (operation, context) => applyOperation(operation, context, Respect_ctx.ContextMap.empty, false, false);
