...
describe(
  ...
  [
    describe(
      ...
      [
        it_w(
          "Doesn't execute example code when setup code fails",
          (_, don) => {
            let lines = ref([]);
        ...
        it_w(
          "Executes the setup code before the example",
          (_, don) => {
            let lines = ref([]);
            let append = (line) => lines := lines^ @ [line];
            let ex =
        ...
        describe(
          ...
          [
            it_w(
              "Runs the setup before each example",
              (_, don) => {
                let lines = ref([]);
                let append = (line) => lines := lines^ @ [line];
                let ex =
        ...
        describe(
          ...
          [
            it_w(
              "Runs the setups from outermost to innermost group",
              (_, don) => {
                let lines = ref([]);
                let append = (line) => lines := lines^ @ [line];
                let innerGroup =
    ...
    describe(
      "ExampleGroup has metadata",
      [
        it_w(
          "Initializes the metadata on the test context",
          (_, don) => {
            let lines = ref([]);
            let append = (line) => lines := lines^ @ [line];
            let grp =
    ...
    describe(
      "Parent group has metadata",
      [
        it_w(
          "uses the metadata closest to the example",
          (_, don) => {
            let lines = ref([]);
            let append = (line) => lines := lines^ @ [line];
            let innerGroup =
